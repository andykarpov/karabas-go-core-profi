...
entity soft_switches is
	...
	MODE : out std_logic_vector(1 downto 0) := "00"; -- dummy
...
--				when x"05" => VIDEO <= SOFTSW_COMMAND(0);
				when x"05" => VSYNC <= SOFTSW_COMMAND(0);
				when x"06" => TURBO <= SOFTSW_COMMAND(1 downto 0);
				when x"07" => SWAP_FDD <= SOFTSW_COMMAND(0);
				when x"08" => JOY_TYPE_L <= SOFTSW_COMMAND(2 downto 0);
				when x"09" => JOY_TYPE_R <= SOFTSW_COMMAND(2 downto 0);
--				when x"0B" => MODE <= SOFTSW_COMMAND(1 downto 0);
				when x"0A" => DIVMMC_EN <= SOFTSW_COMMAND(0);
				when x"0B" => NEMOIDE_EN <= SOFTSW_COMMAND(0);
				when x"0C" => KB_TYPE <= SOFTSW_COMMAND(0);
				when x"0D" => PAUSE <= SOFTSW_COMMAND(0);
				when x"0E" => GS_RESET <= SOFTSW_COMMAND(0);
